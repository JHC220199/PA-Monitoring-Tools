...
def main():
    ...
    # Re-filter existing questions against current filter (removes stale false positives)
    before = len(data['questions'])
    data['questions'] = [q for q in data['questions'] if is_prs(q.get('question', ''))]
    removed = before - len(data['questions'])
    if removed:
        print(f"Cleaned {removed} questions that no longer pass the filter")
 
    # Index existing questions by ID for fast lookup
    existing = {q['id']: q for q in data['questions']}
 
    # Fetch window:
    # - First run (no existing data): 135 days — covers the full spreadsheet period
    #   back to 2nd January 2026. This backfill only happens once.
    # - Daily runs: 14 days to catch new questions + recently answered questions
    if data['lastUpdated']:
        from_date = (datetime.now() - timedelta(days=14)).strftime('%Y-%m-%d')
    else:
        from_date = (datetime.now() - timedelta(days=135)).strftime('%Y-%m-%d')
    ...
    for i, kw in enumerate(KEYWORDS, 1):
        ...
        while True:
            ...
            if results and new > 0:
                print(f"  [{i}/{len(KEYWORDS)}] '{kw}' +{new} (pool: {len(seen)})")
            skip += 100
    ...
    for qid, raw in seen.items():
        ...
        if qid in existing:
            ...
            rec = existing[qid]
            if rec.get('answered'):
                continue  # already have an answer, skip
            if not raw.get('dateAnswered') or not raw.get('answerText'):
                continue  # still unanswered
            # Answer has come in!
            answer_text  = raw.get('answerText', '')
            is_holding   = bool(raw.get('answerIsHolding'))
            answering_id = raw.get('answeringMemberId')
            answering_name = ''
            answering_party = ''
            if answering_id:
                m = get_member(answering_id)
                if m:
                    answering_name = m.get('nameDisplayAs', '')
                    po = m.get('latestParty')
                    if po:
                        answering_party = PARTY_MAP.get(po.get('name',''),
                                          po.get('abbreviation','') or po.get('name',''))
            print(f"  Answer found: {rec['memberName']}: {rec['question'][:70]}...")
            summary = summarise_answer(rec['question'], answer_text, is_holding)
            rec.update({
                'answered':        True,
                'isHolding':       is_holding,
                'dateAnswered':    fmt_date(raw['dateAnswered']),
                'answeringMember': answering_name,
            ...
            if answered and raw.get('answeringMemberId'):
                am = get_member(raw['answeringMemberId'])
                if am:
                    answering_name = am.get('nameDisplayAs','')
                    apo = am.get('latestParty')
                    if apo:
            ...
            new_questions.append({
                'id':              raw['id'],
                'uin':             raw.get('uin',''),
                'memberName':      m['nameDisplayAs'] if m else f"Member {raw['askingMemberId']}",
                'party':           party,
                'house':           raw.get('house',''),
                'question':        raw.get('questionText',''),
                'dateTabled':      fmt_date(raw.get('dateTabled','')),
                'dateTabledRaw':   raw.get('dateTabled','')[:10],
                'dateForAnswer':   f"Due {fmt_date(da)}" if da and not answered else '',
                'weekCommencing':  mon.isoformat(),
                'url':             q_url(raw.get('dateTabled',''), raw.get('uin','')),
                'answered':        answered,
                'isHolding':       is_holding if answered else False,
                'dateAnswered':    fmt_date(raw['dateAnswered']) if answered else None,
                'answeringMember': answering_name,
                'answeringParty':  answering_party,
                'answerSummary':   summary,
            })
 
    # ── Check older unanswered questions individually ─────────────────────────
    # Questions outside the 14-day daily window won't be re-fetched by the
    # keyword search above. Check them one-by-one using their UIN so their
    # answers are still captured. Once answered, they're never checked again.
    daily_cutoff = (datetime.now() - timedelta(days=14)).strftime('%Y-%m-%d')
    old_unanswered = sorted(
        [q for q in data['questions']
         if not q.get('answered')
         and q.get('dateTabledRaw', '') < daily_cutoff
         and q.get('uin')],
        key=lambda q: q.get('dateTabledRaw', '')
    )[:30]  # check up to 30 per run
 
    if old_unanswered:
        print(f"\nChecking {len(old_unanswered)} older unanswered questions...")
        for q in old_unanswered:
            time.sleep(1.5)
            params = urlencode({
                'searchTerm':     q['uin'],
                'tabledWhenFrom': q['dateTabledRaw'],
                'tabledWhenTo':   q['dateTabledRaw'],
                'take': 5,
            })
            d = http_get(f'{QUESTIONS_API}?{params}')
            ...
            for r in d['results']:
                raw = r['value']
                if (raw.get('id') == q['id']
                        and raw.get('dateAnswered')
                        and raw.get('answerText')):
                    answer_text  = raw.get('answerText', '')
                    is_holding   = bool(raw.get('answerIsHolding'))
                    answering_id = raw.get('answeringMemberId')
                    answering_name = ''
                    ...
                    if answering_id:
                        ...
                        if m:
                            ...
                            po = m.get('latestParty')
                            if po:
                                answering_party = PARTY_MAP.get(
                                    po.get('name', ''),
                                    po.get('abbreviation', '') or po.get('name', ''))
                    summary = summarise_answer(q['question'], answer_text, is_holding)
                    q.update({
                        'answered':        True,
                        'isHolding':       is_holding,
                        'dateAnswered':    fmt_date(raw['dateAnswered']),
                    ...
                    answer_updates.append(q)
                    print(f"  Answered: {q['memberName']}: {q['question'][:60]}...")
                    break
    ...
    for q in data['questions']:
        ...
        if 'answerSummary' not in q:
            q['answerSummary'] = None
        if 'answeringMember' not in q:
